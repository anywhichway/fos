(function () {
    ...
    if (typeof (module) !== "undefined") {
        ...
        class ServerResponse {
            constructor() {
                ...
                this.promise.body = "";
                Object.keys(ServerResponse.prototype).forEach(key => Object.defineProperty(this.promise, key, {
                    enumerable: false,
                    configurable: true,
                    writable: true,
                    value: ServerResponse.prototype[key]
                }))
                return this.promise;
            }

            addTrailers() {
            } // Adds HTTP trailing headers
            end(text = "") {
                this.body += text;
                this.finished = true;
                this.resolve();
        ...
        createServer = async handler => {
            self.addEventListener('fetch', async event => {
                ...
                event.respondWith(new Response(response.body, {
                    ...
                    statusText: response.statusText || response.statusMessage,
    ...
    function toScript(object, {server}, parent = "") {
        function fos(options) {
            fos._options = options;
            return fos;
        };
    ...
    class FOS {
        constructor(functions, {allow, name="FOS", before, after, done, middleware,log} = {}) {
                        ...
                        while ((key = parts.shift()) && (node = node[key])) {
                            if (parts.length === 0) {
                                try {
                                    ...
                                    result = await node.apply({
                                        request,
                                        response
                                    }, fromJSON(request.query["arguments"]));
        ...
        static(path, {location = "", defaultFile = "index.html", mimeTypes = {}} = {}) {
            mimeTypes = {html: "text/html", js: "application/javascript", ...mimeTypes};
            let fs,
                normalizePath,
                resolvePath;
            try {
                if (typeof (require) === "function") {
                    ...
                    resolvePath = (...args) => resolve(process.cwd(), ...args);
                    console.log(process.cwd())
                }
            } catch (e) {
                ;
            }
            if (!fs || !resolvePath || !normalizePath) return; // no-op if not on server where require("fs") works
            this.route(path).get(async (request, response) => {
                ...
                return new Promise(resolve => {
                    //console.log(normalizePath(location + "/" + url));
                    const path = resolvePath(normalizePath(location + "/" + url).slice(1));
                    if(this.settings.log) console.log(path);
                    fs.readFile(path, async (err, data) => {
                        if (err) {
                            ...
                            if (type) {
                                ...
                                } = typeof type === "string" ? {contentType: type} : type;
                                response.writeHead(200, {'Content-Type': contentType});
                                response.write(await transform(data, request, response));
                            } else {
